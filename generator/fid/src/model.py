...
class FiDT5(transformers.T5ForConditionalGeneration):
    def __init__(self, config):
        ...
        self.wrap_encoder()  # 把T5的encoder包装成Fid，即把原本模型内部的encoder换成自己的encoder
    ...
    def forward(self, input_ids=None, attention_mask=None, **kwargs):
        if input_ids != None:
            ...
            if input_ids.dim() == 3:
                self.encoder.n_passages = input_ids.size(1)  # n_passages
            input_ids = input_ids.view(input_ids.size(0), -1)  # B x (N * L)  4*2000
        if attention_mask != None:
            attention_mask = attention_mask.view(attention_mask.size(0), -1)  # B x (N * L)
    ...
    def wrap_encoder(self, use_checkpoint=False):
        ...
        """
        self.encoder = EncoderWrapper(self.encoder, use_checkpoint=use_checkpoint)  # use_checkpoint为False， EncoderWrapper作用不大
    ...
    def load_t5(self, state_dict):
        self.unwrap_encoder()  # 首先把包装的fid encoder解包装，以便加载初始值
...
class EncoderWrapper(torch.nn.Module):
    """
    Encoder Wrapper for T5 Wrapper to obtain a Fusion-in-Decoder model. use_checkpoint=False时作用不大
    """
    ...
    def forward(self, input_ids=None, attention_mask=None, **kwargs,):
        ...
        bsz, total_length = input_ids.shape  # 4 2000
        passage_length = total_length // self.n_passages  # 2000//10 = 200
        input_ids = input_ids.view(bsz*self.n_passages, passage_length)  # 40*200
        attention_mask = attention_mask.view(bsz*self.n_passages, passage_length)
        # 设置成return_dict=False
        # kwargs['return_dict'] = False
...
class CheckpointWrapper(torch.nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, position_bias, **kwargs):  # 40*200*768  40*1*1*200
        if self.use_checkpoint and self.training:  # use checkpoint=false
...
def apply_checkpoint_wrapper(t5stack, use_checkpoint):
    """
    Wrap each block of the encoder to enable checkpointing 但是一般use_checkpoint为False，forward中不会有改变
    """
    block = []
