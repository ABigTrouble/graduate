...
class OurDataCollatorWithPadding:
    ...
    def negative_sample_mask(self, target_sent, dtype=torch.bool): # 512*512的矩阵，0表示是正样本
    ...
    def __call__(self, batch) -> Dict[str, torch.Tensor]:
        bs = len(batch)
        assert bs
        num_sent = len(batch[0]['input_ids']) # 2，因为一个batch里面有2句相同的
        pad_token = self.pad_token_id

        flat_input_ids = []
        for sample in batch:  # 将样本对展平 bs*2
            for i in range(num_sent):
                flat_input_ids.append(sample['input_ids'][i])

        flat_idf_weights = []
        for input_ids in flat_input_ids:  # idx的权重，特殊的sep和 cls为权重为0，其他：1 
            cur_idf_weights = [self.idf_dict[id] for id in input_ids]
            flat_idf_weights.append(cur_idf_weights)  # idf表示id权重

        padded, lens, mask = self.padding_func(flat_input_ids, pad_token, dtype=torch.long) # pad
        ...
        negative_sample_mask = self.negative_sample_mask(target_sent)  # 防止一个句子在一个batch中出项多次
...
def tok_sentences(tokenizer, sentences, has_hard_neg, total, max_length=None):
    ...
    if has_hard_neg:
        ...
        for key in sent_features:   # 构建正样本对
