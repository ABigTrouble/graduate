...
class RetrievalModel(PreTrainedModel):
    ...
    def __init__(
        ...
        self._model = get_model(self.model_type, self.num_layers, self.all_layers) # 主要是去除某些层，防止如果num_layer!=12, 使用T5EncoderModel，不添加特定任务头，只输出最后的隐藏状态
    ...
    def calc_pair_tok_embedding(self, input_ids, attention_mask, lengths=None, input_idf=None, num_sent=None):
        ...
        batch_size = int(input_ids.shape[0] / num_sent)   # 实际的bs
        # calc embeddings
        out = self._model(input_ids, attention_mask=attention_mask, output_hidden_states=self.all_layers)
        if self.all_layers:
            emb = torch.stack(out[-1], dim=2)
        ...
        ref_emb, hyp_emb = split_to_pair(emb)  # 将正样本对配对
        ref_att_mask, hyp_attn_mask = split_to_pair(attention_mask)  # 两个都是 64*32(句子长度)
        ref_len, hyp_len = split_to_pair(lengths)  # 64, 句子的长度
        ref_idf, hyp_idf = split_to_pair(input_idf)

        # pad to calculate greedy cos idf
        # emb_pad: padding with value 2
        ref_emb.masked_fill_(~ref_att_mask.bool().unsqueeze(-1), 2)  # 将mask为0的置为2
        ...
        ref_pad_mask = length_to_mask(ref_len, max_len)  # 生成pad了之后的mask，mask为true的长度就等于原本句子的长度
    ...
    def forward(self, input_ids=None, attention_mask=None, negative_sample_mask=None,
        ...
        if 'cls_distance' in self.model_args.sim_func:
            ...
            m_ref_emb = (ref_emb * ref_pad_mask.unsqueeze(-1)).sum(dim=1) / ref_pad_mask.sum(-1).unsqueeze(-1)  # 把pad的部分置为0，然后/len
            ...
            if 'cosine' in self.model_args.sim_func:
                ...
                sim_score = cos_sim(m_ref_emb.unsqueeze(1), m_hyp_emb.unsqueeze(0))  # #相似度计算，bs*1*hidden 和 1*bs*hidden 得到 一个batch内的相似度矩阵 bs * bs
            ...
            if pairwise_similarity: # pairwise score only
                ...
                labels = torch.arange(sim_score.size(0)).long().to(sim_score.device) # 创建label，0，1，2，3...
                # mask the conflict negative examples
                sim_score.masked_fill_(~negative_sample_mask, -1e10)  # negative_sample_mask判断是否一个batch里面有多个相同的，置为0避免影响
