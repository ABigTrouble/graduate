...
class CLTrainer(Trainer):
    ...
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None):
        ...
        """
        # This might change the seed so needs to run first. 用于设置超参数搜索
        ...
        train_dataset_is_sized = isinstance(self.train_dataset, collections.abc.Sized) # 检查是否是后者的对象，如果是则说明可以用len()
        ...
        self.state = TrainerState()  # 用于跟踪和记录训练的不同阶段和状态。
        ...
        if self.use_apex:  # pass
            model, self.optimizer = amp.initialize(model, self.optimizer, opt_level=self.args.fp16_opt_level)

        # Multi-gpu training (should be after apex fp16 initialization)
        if self.args.n_gpu > 1:   # pass
            model = torch.nn.DataParallel(model)

        # Distributed training (should be after apex fp16 initialization)
        if self.sharded_dpp:   # pass
        ...
        if not self.args.ignore_data_skip: # checkpoint的已训练的epoch
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            inputs = None
            last_inputs = None
            for step, inputs in enumerate(epoch_iterator):  # input是已经展开了512->1024
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    if self.args.max_grad_norm is not None and self.args.max_grad_norm > 0 and not self.deepspeed:
                        ...
                        else:  # 具体来说，这个函数会计算所有梯度的范数（L2 范数），然后根据指定的最大梯度范数（self.args.max_grad_norm）对梯度进行缩放，以确保梯度的整体范数不超过指定的阈值。
